...
lazy_static! {
    // Counter for throughput (total ops)
    // Prometheus will calculate the rate()
    pub static ref TOTAL_OPS: IntCounterVec = register_int_counter_vec!(
        ...
        &["tenant_id"] // Graph each core separately
    ).unwrap();
    ...
    T: PacketTx + PacketRx + Display + Clone + 'static,
{
    ...
    fn recv(&mut self) {
        ...
        if curr_cycles > self.log_last_time && (curr_cycles - self.log_last_time) > 240_000_000 {
            ...
            RPC_FRACTION.with_label_values(&[&self.tenant_id]).set((self.ext_p as f64) / 100.0);            
            // 3. Calculate and Update Tail Latency
